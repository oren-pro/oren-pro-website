import React from 'react';

const AboutTopStrip = () => (
    <div className="about__topstrip__box">
        <div className="about__topstrip__gray__bg" />
        <img className="about__topstrip__bg" src="https://res.cloudinary.com/dccqlnk3t/image/upload/v1534143803/about-top_h26efi.jpg" />
        <img className="about__topstrip__header" src="/images/aboutpage/about-top-header.svg" />
        <div className="about__topstrip__text Heebo-Medium" dir="rtl">אורן ורינת.</div>
    </div>
);

export default AboutTopStrip;